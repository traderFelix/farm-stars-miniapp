...
@router.post("/{channel_id}/views/add", response_model=TaskChannelDetailResponse)
async def add_task_channel_views_route(
        channel_id: int,
        payload: TaskChannelAddViewsRequest,
):
    db = await get_db()
    try:
        return await add_channel_views(
            db,
            channel_id=channel_id,
